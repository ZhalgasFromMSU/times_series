...
def main(data):
    ...
    cost_smooth = np.array([np.mean(tmp_X.diagonal(tmp_X.shape[1] - 1 - i)) for i in range(len(data))])
    ...
    return res, cost_smooth


if __name__ == '__main__':
    plt.plot(main(pd.read_csv('data.csv', delimiter=';', names=['date', 'cost']))[0])
    plt.show()
